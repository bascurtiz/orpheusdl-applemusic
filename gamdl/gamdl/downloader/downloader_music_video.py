...
class AppleMusicMusicVideoDownloader(AppleMusicBaseDownloader):
    def __init__(
        ...
        # Pass the decryption IP to the API for the license exchange
        if hasattr(self, 'wrapper_decrypt_ip') and self.wrapper_decrypt_ip:
            if hasattr(self.interface, 'apple_music_api'):
                self.interface.apple_music_api.wrapper_decrypt_ip = self.wrapper_decrypt_ip
