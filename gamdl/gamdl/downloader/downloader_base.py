...
class AppleMusicBaseDownloader:
    ...
    def _initialize_binary_paths(self):
        def resolve_path(provided_path):
            ...
            return shutil.which(provided_path)

        self.full_nm3u8dlre_path = resolve_path(self.nm3u8dlre_path)
        self.full_mp4decrypt_path = resolve_path(self.mp4decrypt_path)
        self.full_ffmpeg_path = resolve_path(self.ffmpeg_path)
        self.full_mp4box_path = resolve_path(self.mp4box_path)
        self.full_amdecrypt_path = resolve_path(self.amdecrypt_path)
