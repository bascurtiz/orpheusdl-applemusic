...
async def decrypt_samples(
    ...
    try:
        ...
        raise ConnectionError(f"Timeout connecting to decryption wrapper at {wrapper_ip}")
    ...
    try:
        ...
        for i, sample in enumerate(samples):
            ...
            if truncated_len > 0:
                ...
                # Read decrypted data with timeout to prevent hangs
                try:
                    decrypted_sample = await asyncio.wait_for(reader.readexactly(truncated_len), timeout=30.0)
                except asyncio.TimeoutError:
                    raise IOError(f"Timeout waiting for decrypted data from wrapper ({truncated_len} bytes)")
