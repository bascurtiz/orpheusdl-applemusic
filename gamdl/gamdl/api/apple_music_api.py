...
class AppleMusicApi:
    ...
    async def create_from_netscape_cookies(
        ...
        **kwargs,
        ...
        # Sort cookies by path length descending to prioritize more specific paths if multiple exist
        sorted_cookies = sorted(
            [c for c in cookies if c.name == "media-user-token" and c.domain == APPLE_MUSIC_COOKIE_DOMAIN],
            key=lambda c: len(c.path or ""),
            reverse=True
        )
        if sorted_cookies:
            media_user_token = sorted_cookies[0].value
    ...
    async def get_license_exchange(
        ...
        if hasattr(self, 'wrapper_decrypt_ip') and self.wrapper_decrypt_ip:
            payload["wrapper-decrypt-ip"] = self.wrapper_decrypt_ip
        ...
        if response.status_code != 200:
            error_msg = f"HTTP error {response.status_code} during license exchange: {response.text}"
            logger.error(error_msg)
            raise Exception(error_msg)

        license_exchange = safe_json(response)
        if not "license" in license_exchange:
            error_msg = f"License not found in response: {response.text}"
            logger.error(error_msg)
            raise Exception(error_msg)
            
        logger.debug(f"License exchange successful")
