...
class AppleMusicSongInterface(AppleMusicInterface):
    ...
    async def get_stream_info(
        ...
        # Parse bit depth and sample rate from flavor string (e.g., "audio-alac-stereo-24-96000-24")
        audio_flavor = playlist["stream_info"].get("audio", "")
        if "alac" in audio_flavor:
            matches = re.findall(r"-(\d+)-(\d+)-(\d+)", audio_flavor)
            if matches:
                stream_info.bit_depth = int(matches[0][0])
                stream_info.sample_rate = int(matches[0][1])
        elif "stereo-" in audio_flavor: # AAC
            matches = re.findall(r"-(\d+)$", audio_flavor)
            if matches:
                 stream_info.sample_rate = 44100 # Default for Apple AAC
    ...
    def _get_playlist_from_codec(
        ...
        if not matching_playlists:
            # Fallback: If specific ALAC tier not found, try generic ALAC before giving up
            if codec in (SongCodec.ALAC_LOSSLESS, SongCodec.ALAC_HI_RES):
                matching_playlists = [
                    playlist
                    for playlist in m3u8_data["playlists"]
                    if re.fullmatch(
                        SONG_CODEC_REGEX_MAP[SongCodec.ALAC.value], playlist["stream_info"]["audio"]
                    )
                ]
        ...
        # Sort by bandwidth. 
        # For Hi-Res, we want the highest quality.
        # For standard Lossless, if we are in fallback, we might prefer the lowest (closest to 16-bit)
        if codec == SongCodec.ALAC_LOSSLESS:
            return min(
                matching_playlists,
    ...
    async def get_decryption_key_legacy(
        ...
        try:
            ...
            content_keys = [i for i in cdm.get_keys(cdm_session) if i.type == "CONTENT"]
            if not content_keys:
                raise Exception("No content keys found in license response. This usually indicates a license exchange failure or an incompatible device.")
            decryption_key = content_keys[0]
        finally:
